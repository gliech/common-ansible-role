...
guests = [
    ...
    { name: 'ubuntu', box: 'ubuntu/focal64' },
    { name: 'debian-testing', box: 'debian/testing64', hostvars: { ansible_python_interpreter: '/usr/bin/python3' } },
    { name: 'debian10', box: 'debian/buster64', hostvars: { ansible_python_interpreter: '/usr/bin/python3' } },
...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
    ...
    config.ssh.private_key_path = [
        Vagrant::user_data_path().join("insecure_private_key").to_s,
        'vagrant/testadmin3.private_key'
    ]
        ...
        config.vm.define guest[:vagrant_name] do |machine|
            ...
            if ansible_mode == 'guest' and guest[:name] == 'provisioner'
                ...
                machine.vm.synced_folder "./.vagrant/machines", "/machines",
                    type: "rsync",
                    rsync__args: [ "--verbose", "--archive", "--delete", "-zz", "--copy-links", "--chmod=D700,F600"]
            else
                machine.vm.synced_folder '.', '/vagrant',
                    type: "rsync",
